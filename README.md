...
```
  ...
  Items & Equipment: 110  (Tools 18, Gear 19, Materials 21, ...)
```
...
| Locations | regions, cities, areas | Regions, Cities, Areas |
...
- The `X` / `X_(Mechanics)` / `X_Items` triples (Work Efficiency, Item Finding, Double
  Action…) are **attributes**: the bare page goes to Game Systems, the `_Items` page is an
  item index under Items & Equipment.
